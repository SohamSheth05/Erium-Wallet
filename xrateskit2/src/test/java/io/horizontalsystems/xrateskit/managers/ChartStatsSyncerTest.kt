package io.horizontalsystems.xrateskit2.managers

import com.nhaarman.mockitokotlin2.*
import io.horizontalsystems.xrateskit2.core.*
import io.horizontalsystems.xrateskit2.entities.ChartInfoKey
import io.horizontalsystems.xrateskit2.entities.ChartPointEntity
import io.horizontalsystems.xrateskit2.entities.ChartType
import io.horizontalsystems.xrateskit2.rates.LatestRatesScheduler
...
object ChartStatsSyncerTest : Spek({
    ...
    describe("#sync") {
        context("when latest rates fetched from API") {
            ...
            beforeEach {
                whenever(statsProvider.getChartPointsAsync(coin, currency, chartType)).thenReturn(
                    Single.just(stats)
                )
            }
    ...
    describe("#subscribe(onFire)") {
        ...
        context("when subject holders has observers") {
            ...
            context("when no chart stats in DB") {
                beforeEach {
                    whenever(storage.getLatestChartPoints(coin, currency, chartType)).thenReturn(
                        null
                    )
                }
            ...
            context("when chart stats exists in DB but expired") {
                ...
                beforeEach {
                    ...
                    val chartStats = ChartPointEntity(
                        chartType,
                        coin,
                        currency,
                        BigDecimal.TEN,
                        today - (chartType.seconds + 1)
                    )
                    whenever(storage.getLatestChartPoints(coin, currency, chartType)).thenReturn(
                        chartStats
                    )
