...
import io.horizontalsystems.xrateskit2.RxTestRule
import io.horizontalsystems.xrateskit2.XRatesDataSource
import io.horizontalsystems.xrateskit2.core.*
import io.horizontalsystems.xrateskit2.entities.LatestRate
import io.horizontalsystems.xrateskit2.rates.LatestRateSyncer
    ...
    describe("#sync") {
        ...
        context("when data source set") {
            ...
            it("fetches latest rate from API") {
                whenever(
                    rateProvider.getLatestRatesAsync(
                        coins,
                        currency
                    )
            ...
            context("when latest rates fetched from API") {
                beforeEach {
                    whenever(rateProvider.getLatestRatesAsync(coins, currency)).thenReturn(
                        Observable.just(latestRate)
                    )
                }
