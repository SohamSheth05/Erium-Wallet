package io.horizontalsystems.xrateskit2.providers.coingecko
...
import io.horizontalsystems.xrateskit2.core.*
...
class CoinGeckoProvider(
    ...
    private fun getCoinType(providerCoinId: String): CoinType? {
        return providerCoinsManager.getCoinTypes(
            providerCoinId.toLowerCase(Locale.ENGLISH),
            provider
        ).firstOrNull()
    }

    override fun getTopCoinMarketsAsync(
        currencyCode: String,
    ...
    private fun getCoinMarketsSingle(
        currencyCode: String,
        fetchDiffPeriod: TimePeriod,
        itemsCount: Int,
        pageNumber: Int,
    ...
    override fun getCoinMarketsAsync(
        coinTypes: List<CoinType>,
        currencyCode: String,
        fetchDiffPeriod: TimePeriod
    ): Single<List<CoinMarket>> {
        val providerCoinIds =
            providerCoinsManager.getProviderIds(coinTypes, provider).filterNotNull()
    ...
    private fun convertCoinGeckoCoinMarket(
        responseCoinMarket: CoinGeckoService.Response.CoinMarket,
        currencyCode: String,
        fetchDiffPeriod: TimePeriod
    ): CoinMarket? {
        ...
        return factory.createCoinMarket(
            coinData = CoinData(
                type,
                responseCoinMarket.symbol.toUpperCase(Locale.ENGLISH),
                responseCoinMarket.name
            ),
    ...
    override fun getCoinMarketDetailsAsync(
        coinType: CoinType,
        currencyCode: String,
        ...
        return coinGeckoService.coin(providerCoinId, "true", "false", "false")
            .map { coin ->
                ...
                val marketTickers = coin.tickers.map {
                    val base =
                        if (contractAddresses.contains(it.base.toLowerCase(Locale.ENGLISH))) {
                            coin.symbol
                        } else {
                            it.base
                        }
                    ...
                    val target =
                        if (contractAddresses.contains(it.target.toLowerCase(Locale.ENGLISH))) {
                            coin.symbol
                        } else {
                            it.target
                        }

                    CoinGeckoService.Response.Coin.Ticker(
                        base,
                        target,
                        it.market,
                        it.last,
                    ...
                    .map {
                        val imageUrl =
                            coinInfoManager.getExchangeInfo(it.market.identifier)?.imageUrl
                        MarketTicker(
                            it.base,
                            it.target,
                            it.market.name,
                ...
                val rateDiffsPeriod =
                    rateDiffPeriods.map<TimePeriod, Pair<TimePeriod, Map<String, BigDecimal>>> { period ->
                        ...
                        val rateDiffs = rateDiffCoinCodes.map { coinCode ->
                            coinCode to (diffPeriod[coinCode.toLowerCase(Locale.ENGLISH)]
                                ?: BigDecimal.ZERO)
                        }.toMap()
                ...
                CoinMarketDetails(
                    ...
                    rateHigh24h = coin.market_data.high_24h[currencyCodeLowercase]
                        ?: BigDecimal.ZERO,
                    rateLow24h = coin.market_data.low_24h[currencyCodeLowercase] ?: BigDecimal.ZERO,
                    marketCap = coin.market_data.market_cap[currencyCodeLowercase]
                        ?: BigDecimal.ZERO,
                    marketCapDiff24h = coin.market_data.market_cap_change_percentage_24h,
                    marketCapRank = coin.market_data.market_cap_rank,
                    dilutedMarketCap = coin.market_data.fully_diluted_valuation[currencyCodeLowercase]
                        ?: BigDecimal.ZERO,
                    volume24h = coin.market_data.total_volume[currencyCodeLowercase]
                        ?: BigDecimal.ZERO,
    ...
    override fun getChartPointsAsync(chartPointKey: ChartInfoKey): Single<List<ChartPointEntity>> {
        val providerCoinId = getProviderCoinId(chartPointKey.coinType) ?: return Single.error(
            Exception("No CoinGecko CoinId found for ${chartPointKey.coinType}")
        )
        return getCoinMarketCharts(providerCoinId, chartPointKey)
    }

    private fun getCoinMarketCharts(
        providerCoinId: String,
        chartPointKey: ChartInfoKey
    ): Single<List<ChartPointEntity>> {
    ...
    override fun getCoinMarketPointsAsync(
        coinType: CoinType,
        currencyCode: String,
        fetchDiffPeriod: TimePeriod
    ...
    override fun getLatestRatesAsync(
        coinTypes: List<CoinType>,
        currencyCode: String
    ): Single<List<LatestRateEntity>> {
    ...
    private fun getLatestRates(
        coinTypesByCoinGeckoId: Map<String, List<CoinType>>,
        currencyCode: String
    ): Single<List<LatestRateEntity>> {
    ...
    override fun getHistoricalRateAsync(
        coinType: CoinType,
        currencyCode: String,
        timestamp: Long
    ): Single<HistoricalRate> {
        val providerCoinId = getProviderCoinId(coinType)
            ?: return Single.error(Exception("No CoinGecko CoinId found for $coinType"))
