...
class Storage(private val database: Database) : IStorage {
    ...
    override fun getCoinTypesByProviderCoinId(
        providerCoinId: String,
        provider: InfoProvider
    ): List<CoinType> {
        return when (provider) {
            is InfoProvider.CoinGecko -> providerCoinDao.getCoinTypesForCoinGecko(providerCoinId)
            is InfoProvider.CryptoCompare -> providerCoinDao.getCoinTypesForCryptoCompare(
                providerCoinId
            )
            else -> listOf()
        }
    ...
    override fun getHistoricalRate(
        coinType: CoinType,
        currencyCode: String,
        timestamp: Long
    ): HistoricalRate? {
    ...
    override fun getOldLatestRates(
        coinTypes: List<CoinType>,
        currencyCode: String
    ): List<LatestRateEntity> {
    ...
    // GlobalMarketInfo
    override fun getGlobalMarketPointInfo(
        currencyCode: String,
