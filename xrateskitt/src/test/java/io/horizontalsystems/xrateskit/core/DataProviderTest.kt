...
import io.horizontalsystems.xrateskit2.managers.ChartStatSyncer
import io.horizontalsystems.xrateskit2.managers.MarketStatsManager
...
object DataProviderTest : Spek({
    ...
    describe("#getChartInfo") {
        ...
        context("when chart stats exists in DB but expired") {
            ...
            beforeEach {
                whenever(storage.getChartPoints(coin, currency, chartType)).thenReturn(
                    listOf(
                        chartStats
                    )
                )
        ...
        context("when chart stats exists in DB") {
            val chartStats =
                ChartPointEntity(chartType, coin, currency, BigDecimal.TEN, Date().time / 1000 - 1)
            ...
            context("when latest rate also exists in DB") {
                ...
                beforeEach {
                    ...
                    whenever(
                        factory.createChartPoint(
                            chartStats.value,
                            chartStats.timestamp
                        )
                    ).thenReturn(ChartPoint(chartStats.value, chartStats.timestamp))
                    whenever(
                        factory.createChartPoint(
                    ...
                    ).thenReturn(ChartPoint(latestRate.value, latestRate.timestamp))
                }
