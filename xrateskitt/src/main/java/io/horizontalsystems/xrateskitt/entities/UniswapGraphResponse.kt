                ...
                data.asObject()?.let { periodData ->
                    periodData.names().forEach { periodName ->
                        periodData.get(periodName).let {
                            blockData[TimePeriod.valueOf(periodName)] =
                                it.asArray().get(0).asObject().get("number").asString().toLong()
                        }
                    }
                }
            }
...
data class UniswapGraphToken(
    ...
    companion object {
        fun parseData(jsonValue: JsonValue): UniswapGraphToken {
            ...
            val latestRateInETH =
                if (!tokenData.get("derivedETH").isNull) tokenData.get("derivedETH").asString()
                    .toBigDecimal() else BigDecimal.ZERO
            val volumeInUSD =
                if (!tokenData.get("tradeVolumeUSD").isNull) tokenData.get("tradeVolumeUSD")
                    .asString().toBigDecimal() else BigDecimal.ZERO
            val totalLiquidity =
                if (!tokenData.get("totalLiquidity").isNull) tokenData.get("totalLiquidity")
                    .asString().toBigDecimal() else BigDecimal.ZERO
...
data class UniswapGraphTokensResponse(
    ...
    companion object {
        fun parseData(jsonObject: JsonObject): UniswapGraphTokensResponse {
            ...
            return UniswapGraphTokensResponse(
                rates,
                UniswapGraphEthXRateResponse.parseData(jsonObject).rateInUSD
            )
        }
    }
}
        ...
        fun parseData(jsonObject: JsonObject): UniswapGraphEthXRateResponse {
            if (!jsonObject.get("data").isNull) {
                jsonObject.get("data")?.asObject()?.get("bundles")?.let {
                    return UniswapGraphEthXRateResponse(
                        it.asArray()[0].asObject()["ethPriceUSD"].asString().toBigDecimal()
                    )
                }
            }
