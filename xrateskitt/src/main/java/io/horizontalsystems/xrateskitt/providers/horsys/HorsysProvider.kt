package io.horizontalsystems.xrateskit2.providers.horsys
...
class HorsysProvider(
    ...
    override fun getTopTokenHoldersAsync(
        coinType: CoinType,
        itemsCount: Int
    ...
    override fun getTopDefiTvlAsync(
        ...
        val period =
            if (isTimePeriodSupported(fetchDiffPeriod)) fetchDiffPeriod.title else TimePeriod.HOUR_24.title
    ...
    override fun getDefiTvlAsync(coinType: CoinType, currencyCode: String): Single<DefiTvl> {
        ...
        providerCoinsManager.getProviderIds(listOf(coinType), InfoProvider.CoinGecko())
            .firstOrNull()?.let { coinGeckoId ->
            return horsysService.coinDefiTvl(coinGeckoId, currencyCode).map { response ->
                DefiTvl(
                    CoinData(coinType, response.code, response.name),
                    response.tvl,
                    BigDecimal.ZERO,
                    response.tvl_rank ?: 0
                )
    ...
    override fun getDefiTvlPointsAsync(
        coinType: CoinType,
        currencyCode: String,
        timePeriod: TimePeriod
        ...
        providerCoinsManager.getProviderIds(listOf(coinType), InfoProvider.CoinGecko())
            .firstOrNull()?.let { coinGeckoId ->
            return horsysService.defiTvlPoints(coinGeckoId, timePeriod.title, currencyCode)
                .map { responseList ->
