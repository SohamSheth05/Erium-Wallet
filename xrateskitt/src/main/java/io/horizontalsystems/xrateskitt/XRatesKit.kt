...
class XRatesKit(
    ...
    fun getNotificationCoinCode(coinType: CoinType): String? {
        return providerCoinsManager.getProviderIds(listOf(coinType), InfoProvider.CryptoCompare())
            .firstOrNull()
    }
    ...
    fun chartInfoObservable(
        ...
        return chartInfoSyncManager.chartInfoObservable(
            ChartInfoKey(
                coinType,
                currencyCode,
                chartType
    ...
    fun getCoinMarketsByCategoryAsync(
        categoryId: String,
        currencyCode: String,
        fetchDiffPeriod: TimePeriod = TimePeriod.HOUR_24
    ): Single<List<CoinMarket>> {
    ...
    fun getGlobalCoinMarketsAsync(
        currencyCode: String,
        timePeriod: TimePeriod = TimePeriod.HOUR_24
    ): Single<GlobalCoinMarket> {
    ...
    companion object {
        fun create(
            context: Context,
            rateExpirationInterval: String = 60L,
            retryInterval: Long = 30,
            cryptoCompareApiKey: String = "",
            coinsRemoteUrl: String?,
            providerCoinsRemoteUrl: String?
        ): XRatesKit {
            ...
            val providerCoinsResourceProvider =
                buildProviderCoinsResourceProviderImpl(context, providerCoinsRemoteUrl)
            val coinExternalIdsSyncer =
                CoinExternalIdsSyncer(providerCoinsResourceProvider, storage)
            val providerCoinsManager = ProviderCoinsManager(storage, coinExternalIdsSyncer)

            val coinGeckoProvider =
                CoinGeckoProvider(factory, coinInfoManager, providerCoinsManager)
            ...
            val latestRatesSchedulerFactory = LatestRatesSchedulerFactory(
                latestRatesManager,
                coinGeckoProvider,
                rateExpirationInterval,
                retryInterval
            )
            ...
            val chartInfoSchedulerFactory =
                ChartInfoSchedulerFactory(chartInfoManager, coinGeckoProvider, retryInterval)
        ...
        private fun buildProviderCoinsResourceProviderImpl(
            context: Context,
            providerCoinsRemoteUrl: String?
        ...
        private fun buildCoinInfoResourceProvider(
            context: Context,
            coinsRemoteUrl: String?
        ): DataProvider<CoinInfoResource> {
