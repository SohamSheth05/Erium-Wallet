...
class AdapterFactory(
    ...
    fun adapter(wallet: Wallet): IAdapter? {
        val syncMode = initialSyncModeSettingsManager?.setting(
            wallet.coin.type,
            wallet.account.origin
        )?.syncMode

        return when (val coinType = wallet.coin.type) {
            is CoinType.Zcash -> ZcashAdapter(
                context,
                wallet,
                restoreSettingsManager.settings(wallet.account, wallet.coin),
                testMode
            )
            ...
            is CoinType.BitcoinCash -> BitcoinCashAdapter(
                wallet,
                syncMode,
                testMode,
                backgroundManager
            ...
            is CoinType.Bep2 -> BinanceAdapter(
                binanceKitManager.binanceKit(wallet),
                coinType.symbol
            )
            is CoinType.Ethereum -> EvmAdapter(ethereumKitManager.evmKit(wallet.account))
            is CoinType.Dgc -> Eip20Adapter(
                context,
                binanceSmartChainKitManager.evmKit(wallet.account),
                wallet.coin.decimal,
                coinType.address1
            )
            is CoinType.Dsc -> Eip20Adapter(
                context,
                binanceSmartChainKitManager.evmKit(wallet.account),
                wallet.coin.decimal,
                coinType.address2
            )
            is CoinType.Dpc -> Eip20Adapter(
                context,
                binanceSmartChainKitManager.evmKit(wallet.account),
                wallet.coin.decimal,
                coinType.address3
            )
            is CoinType.Erc20 -> Eip20Adapter(
                context,
                ethereumKitManager.evmKit(wallet.account),
                wallet.coin.decimal,
                coinType.address
            )
            is CoinType.BinanceSmartChain -> EvmAdapter(binanceSmartChainKitManager.evmKit(wallet.account))
            is CoinType.Bep20 -> Eip20Adapter(
                context,
                binanceSmartChainKitManager.evmKit(wallet.account),
                wallet.coin.decimal,
                coinType.address
