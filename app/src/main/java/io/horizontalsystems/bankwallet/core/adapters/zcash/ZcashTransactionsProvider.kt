...
class ZcashTransactionsProvider(synchronizer: Synchronizer) {
    ...
    init {
        synchronizer.clearedTransactions.distinctUntilChanged()
            .collectWith(GlobalScope, ::onClearedTransactions)
        synchronizer.pendingTransactions.distinctUntilChanged()
            .collectWith(GlobalScope, ::onPendingTransactions)
    }
    ...
    private val newTransactionsSubject: PublishSubject<List<ZcashTransaction>> =
    ...
    private fun onClearedTransactions(transactions: List<ConfirmedTransaction>) {
        val newTransactions =
            transactions.filter { tx -> tx.minedHeight > 0 && !confirmedTransactions.any { it.id == tx.id } }
    ...
    private fun onPendingTransactions(transactions: List<PendingTransaction>) {
        val newTransactions =
            transactions.filter { tx -> tx.hasRawTransactionId() && !tx.isMined() && !pendingTransactions.any { it.id == tx.id } }
            ...
            try {
                ...
                emitter.onSuccess(
                    allTransactions.subList(
                        fromIndex,
                        min(allTransactions.size, fromIndex + limit)
                    )
                )
