    ...
    companion object {
                    ...
                    if (coinIdColumnIndex >= 0) {
                        ...
                        if (coinId == "BTC") {
                            ...
                            if (derivationColumnIndex >= 0) {
                                val walletDerivation =
                                    walletsCursor.getString(derivationColumnIndex)
                                if (walletDerivation != null) {
                                    try {
                                        val derivation =
                                            AccountType.Derivation.valueOf(walletDerivation)
        ...
        private val addBlockchainSettingsTable: Migration = object : Migration(14, 15) {
            ...
            private fun saveSettings(
                ...
                derivation?.let {
                    insertIntoBlockchainSetting(
                        database,
                        coinType,
                        BlockchainSettingsStorage.derivationSettingKey,
                        it
                    )
                }
                ...
                syncMode?.let {
                    insertIntoBlockchainSetting(
                        database,
                        coinType,
                        BlockchainSettingsStorage.syncModeSettingKey,
                        it
                    )
                }
                ...
                communication?.let {
                    insertIntoBlockchainSetting(
                        database,
                        coinType,
                        BlockchainSettingsStorage.ethereumRpcModeSettingKey,
                        it
                    )
                }
            ...
            private fun insertIntoBlockchainSetting(
                database: SupportSQLiteDatabase,
                coinType: String,
                key: String,
                value: String
        ...
        private val MIGRATION_24_25: Migration = object : Migration(24, 25) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                val walletsCursor =
                    database.query("SELECT * FROM EnabledWallet WHERE coinId = 'BCH'")
        ...
        private val MIGRATION_29_30: Migration = object : Migration(29, 30) {
                    ...
                    if (coinIdColumn >= 0) {
                        ...
                        if (changeColumn >= 0) {
                            ...
                            if (value != PriceAlert.ChangeState.OFF.value) {
                                val topic =
                                    "${coinId.toUpperCase(Locale.ENGLISH)}_24hour_${value}percent"
                                insertUnsubscribeJobs(
                                    database,
                                    coinId,
                                    topic,
                        ...
                        if (trendColumn >= 0) {
                            ...
                            if (value != PriceAlert.TrendState.OFF.value) {
                                val topic =
                                    "${coinId.toUpperCase(Locale.ENGLISH)}_${value}term_trend_change"
                                insertUnsubscribeJobs(
                                    database,
                                    coinId,
                                    topic,
            ...
            fun insertUnsubscribeJobs(
                database: SupportSQLiteDatabase,
                coinCode: String,
                topic: String,
                stateType: SubscriptionJob.StateType,
                changeValue: String
            ) {
                database.execSQL(
                    """
        ...
        private val MIGRATION_30_31: Migration = object : Migration(30, 31) {
            ...
            fun getChangeBody(coinTypeId: String, changeValue: String): String {
                val data =
                    hashMapOf("coin_id" to coinTypeId, "change" to changeValue, "period" to "24h")
        ...
        private fun updateCoinIdInEnabledWallets(
            customCoins: List<Coin>,
            database: SupportSQLiteDatabase
        ) {
