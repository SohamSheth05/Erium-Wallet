...
class BlockchainSettingsStorage(private val appDatabase: AppDatabase) : IBlockchainSettingsStorage {
    ...
    override var bitcoinCashCoinType: BitcoinCashCoinType?
        get() {
            val blockchainSetting = appDatabase.blockchainSettingDao()
                .getSetting(CoinType.BitcoinCash, networkCoinTypeKey)
        ...
        set(newValue) {
            ...
            appDatabase.blockchainSettingDao()
                .insert(BlockchainSetting(CoinType.BitcoinCash, networkCoinTypeKey, newValue.value))
        }


    override fun derivationSetting(coinType: CoinType): DerivationSetting? {
        val blockchainSetting =
            appDatabase.blockchainSettingDao().getSetting(coinType, derivationSettingKey)
        return blockchainSetting?.let {
            DerivationSetting(
                coinType,
                AccountType.Derivation.valueOf(it.value)
            )
        }
    }

    override fun saveDerivationSetting(derivationSetting: DerivationSetting) {
        appDatabase.blockchainSettingDao().insert(
            BlockchainSetting(
                derivationSetting.coinType,
                derivationSettingKey,
                derivationSetting.derivation.value
            )
        )
    }
    ...
    override fun initialSyncSetting(coinType: CoinType): InitialSyncSetting? {
        val blockchainSetting =
            appDatabase.blockchainSettingDao().getSetting(coinType, syncModeSettingKey)
    ...
    override fun saveInitialSyncSetting(initialSyncSetting: InitialSyncSetting) {
        appDatabase.blockchainSettingDao().insert(
            BlockchainSetting(
                initialSyncSetting.coinType,
                syncModeSettingKey,
                initialSyncSetting.syncMode.value
            )
        )
    }
    ...
    override fun ethereumRpcModeSetting(coinType: CoinType): EthereumRpcMode? {
        val setting =
            appDatabase.blockchainSettingDao().getSetting(coinType, ethereumRpcModeSettingKey)
    ...
    override fun saveEthereumRpcModeSetting(ethereumRpcModeSetting: EthereumRpcMode) {
        appDatabase.blockchainSettingDao().insert(
            BlockchainSetting(
                ethereumRpcModeSetting.coinType,
                syncModeSettingKey,
                ethereumRpcModeSetting.communicationMode.value
            )
        )
    }
}
