...
import io.horizontalsystems.bankwallet.core.retrofit.Retrofit
...
import io.horizontalsystems.coinkit2.CoinKit
...
class App : CoreApp() {
    ...
    override fun onCreate() {
        ...
        val adapterFactory = AdapterFactory(
            instance,
            buildConfigProvider.testMode,
            ethereumKitManager,
            binanceSmartChainKitManager,
            binanceKitManager,
            backgroundManager,
            restoreSettingsManager
        )
        ...
        initialSyncModeSettingsManager = InitialSyncSettingsManager(
            coinManager,
            blockchainSettingsStorage,
            adapterManager,
            walletManager
        )
        derivationSettingsManager =
            DerivationSettingsManager(blockchainSettingsStorage, adapterManager, walletManager)
        ethereumRpcModeSettingsManager =
            EthereumRpcModeSettingsManager(blockchainSettingsStorage, adapterManager, walletManager)
        bitcoinCashCoinTypeManager =
            BitcoinCashCoinTypeManager(walletManager, adapterManager, blockchainSettingsStorage)
        ...
        notificationNetworkWrapper =
            NotificationNetworkWrapper(localStorage, networkManager, appConfigProvider)
        notificationManager =
            NotificationManager(NotificationManagerCompat.from(this), localStorage).apply {
                backgroundManager.registerListener(this)
            }
        notificationSubscriptionManager =
            NotificationSubscriptionManager(appDatabase, notificationNetworkWrapper)
        ...
        walletConnectSessionManager =
            WalletConnectSessionManager(walletConnectSessionStorage, accountManager)
        walletConnectManager =
            WalletConnectManager(accountManager, ethereumKitManager, binanceSmartChainKitManager)
