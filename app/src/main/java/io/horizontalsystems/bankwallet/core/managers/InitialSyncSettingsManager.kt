...
class InitialSyncSettingsManager(
    ...
    override fun setting(coinType: CoinType, origin: AccountOrigin?): InitialSyncSetting? {
        val supportedCoinType =
            supportedCoinTypes.firstOrNull { it.coinType == coinType } ?: return null
    ...
    override fun save(setting: InitialSyncSetting) {
        ...
        val walletsForUpdate =
            walletManager.activeWallets.filter { it.coin.type == setting.coinType && it.account.origin == AccountOrigin.Restored }
    ...
    private fun defaultSetting(supportedCoinType: CoinType): InitialSyncSetting {
        return InitialSyncSetting(
            supportedCoinType,
            supportedCoinTypes.first { it.coinType == supportedCoinType }.defaultSyncMode
        )
    }
