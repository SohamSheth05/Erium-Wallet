...
class PriceAlertManager(
    ...
    override fun getAlertStates(coinType: CoinType): Pair<PriceAlert.ChangeState, PriceAlert.TrendState> {
        ...
        return Pair(
            priceAlert?.changeState ?: PriceAlert.ChangeState.OFF, priceAlert?.trendState
    ...
    override suspend fun fetchNotifications() {
        ...
        if (response.code() == HttpURLConnection.HTTP_OK) {
            ...
            val messages =
                responseBody.asJsonObject["messages"].asJsonArray.mapNotNull { jsonElement ->
            ...
            localStorageManager.notificationServerTime =
                responseBody.asJsonObject["server_time"].asNumber.toLong()
    ...
    private fun updateSubscription(
        newAlert: PriceAlert,
        oldChangeState: PriceAlert.ChangeState,
        oldTrendState: PriceAlert.TrendState
    ) {
        val jobs = mutableListOf<SubscriptionJob>()

        if (oldChangeState != newAlert.changeState) {
            val subscribeJob = getChangeSubscriptionJob(
                newAlert.coinType,
                newAlert.changeState,
                SubscriptionJob.JobType.Subscribe
            )
            val unsubscribeJob = getChangeSubscriptionJob(
                newAlert.coinType,
                oldChangeState,
                SubscriptionJob.JobType.Unsubscribe
            )
            ...
            val subscribeJob = getTrendSubscriptionJob(
                newAlert.coinType,
                newAlert.trendState,
                SubscriptionJob.JobType.Subscribe
            )
            val unsubscribeJob = getTrendSubscriptionJob(
                newAlert.coinType,
                oldTrendState,
                SubscriptionJob.JobType.Unsubscribe
            )
    ...
    */
    ...
    companion object {
        fun getChangeSubscriptionJob(
            coinType: CoinType,
            changeState: PriceAlert.ChangeState,
            subscribeType: SubscriptionJob.JobType
        ): SubscriptionJob {
            val data = hashMapOf(
                "coin_id" to coinType.ID,
                "percent" to changeState.getIntValue(),
                "period" to "24h"
            )
