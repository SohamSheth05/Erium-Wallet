...
class SendAmountPresenterHelper(
            ...
            SendModule.InputType.COIN -> {
                rate?.let {
                    numberFormatter.formatFiat(
                        coinAmount?.times(it) ?: BigDecimal.ZERO,
                        baseCurrency.symbol,
                        2,
                        2
                    )
    ...
    fun getCoinAmount(
        ...
        return when (inputType) {
            SendModule.InputType.CURRENCY -> rate?.let {
                amount?.divide(
                    it,
                    coinDecimal,
    ...
    fun decimal(inputType: SendModule.InputType) =
        if (inputType == SendModule.InputType.COIN) coinDecimal else currencyDecimal
