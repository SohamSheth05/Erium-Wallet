...
class PrivacySettingsPresenter(
    ...
    private val communicationSettingsViewItems: List<PrivacySettingsViewItem> = listOf(
        PrivacySettingsViewItem(
            "Ethereum",
            interactor.ether,
            CommunicationModeSettingType(CommunicationMode.Infura),
            enabled = ethereumCommunicationModeCanBeChanged(),
            listPosition = ListPosition.First
        ),
        PrivacySettingsViewItem(
            "BSC",
            interactor.binanceSmartChain,
            CommunicationModeSettingType(CommunicationMode.Nariox),
            enabled = false,
        ...
        PrivacySettingsViewItem(
            "Binance",
            interactor.binance,
            CommunicationModeSettingType(CommunicationMode.BinanceDex),
            enabled = false,
    ...
    override fun didSwitchTorEnabled(checked: Boolean) {
        ...
        if (checked) {
            ...
            if (interactor.ethereumConnection().communicationMode != CommunicationMode.Infura) {
                ...
                openedPrivacySettings =
                    communicationSettingsViewItems.find { it.coin.type == CoinType.Ethereum }
        ...
        when (settingType) {
            is CommunicationModeSettingType -> {
                ...
                if (item.coin == interactor.ether) {
                    ...
                    view?.showCommunicationSelectorDialog(
    ...
    override fun proceedWithCommunicationModeChange(
        coin: Coin,
    ...
    private fun ethereumCommunicationModeCanBeChanged() =
        interactor.ethereumCommunicationModes.size > 1
    ...
    private fun onSelectCommunicationModeByTor() {
        ...
        val walletsToUpdate =
            interactor.wallets.filter { it.coin.type == CoinType.Ethereum || it.coin.type is CoinType.Erc20 }
    ...
    private fun updateCommunicationMode(coin: Coin, communicationMode: CommunicationMode) {
        (openedPrivacySettings?.settingType as? CommunicationModeSettingType)?.selected =
            communicationMode
