...
class CoinSettingsViewModel(
    ...
    ): BlockchainSettingsModule.Config {
            ...
            viewItems = allDerivations.map { derivation ->
                BottomSheetSelectorViewItem(
                    ...
                    subtitle = Translator.getString(
                        ...
                        (derivation.addressPrefix(coin.type) ?: "")
                    )
                )
            },
            description = Translator.getString(
                R.string.AddressFormatSettings_Description,
                coin.title
            )
        )
    }
    ...
    fun onSelect(indexes: List<Int>) {
        ...
        when (request.type) {
            is CoinSettingsService.RequestType.Derivation -> {
                service.selectDerivations(
                    indexes.map { request.type.allDerivations[it] },
                    request.coin
                )
            }
            is CoinSettingsService.RequestType.BCHCoinType -> {
