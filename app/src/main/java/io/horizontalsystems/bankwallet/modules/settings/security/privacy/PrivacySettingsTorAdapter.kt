        ...
        fun bind(torStatus: TorStatus, checked: Boolean) {
            ...
            when (torStatus) {
                ...
                TorStatus.Closed -> {
                    ...
                    subtitleText.text =
                        containerView.context.getString(R.string.TorPage_ConnectionClosed)
                }
            }

        }

        private fun getTint(color: Int) =
            containerView.context?.let { ColorStateList.valueOf(ContextCompat.getColor(it, color)) }


        companion object {
            const val layout = R.layout.view_holder_tor_control

            fun create(parent: ViewGroup, onSwitch: (isChecked: Boolean) -> Unit) =
                TorControlViewHolder(inflate(parent, layout, false), onSwitch)
        }
