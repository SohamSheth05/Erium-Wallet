...
class CoinService(
    ...
    val coinDetailsStateObservable: BehaviorSubject<CoinDetailsState> =
        BehaviorSubject.createDefault(CoinDetailsState.Loading)
    val alertNotificationUpdatedObservable: BehaviorSubject<Unit> =
    ...
    var lastPoint: LastPoint? = xRateManager.latestRate(coinType, currency.code)
        ?.let { LastPoint(it.rate, it.timestamp, it.rateDiff24h) }
        set(value) {
            field = value
    ...
    fun getCoinDetails(rateDiffCoinCodes: List<String>, rateDiffPeriods: List<TimePeriod>) {
        ...
        xRateManager.coinMarketDetailsAsync(
            coinType,
            currency.code,
            rateDiffCoinCodes,
            rateDiffPeriods
