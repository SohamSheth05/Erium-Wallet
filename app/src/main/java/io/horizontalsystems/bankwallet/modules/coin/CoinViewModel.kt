...
import io.horizontalsystems.xrateskit2.entities.CoinMarketDetails
...
class CoinViewModel(
    ...
    private fun updateCoinDetails() {
        ...
        coinDetailsLiveData.postValue(
            factory.createCoinDetailsViewItem(
                coinDetails,
                service.currency,
                coinCode,
                rateDiffCoinCodes,
                rateDiffPeriods,
                getContractInfo(coinDetails),
                service.guideUrl
            )
        )
    ...
    private fun getContractInfo(coinDetails: CoinMarketDetails): ContractInfo? =
        when (val coinType = coinDetails.data.type) {
            is CoinType.Erc20 -> ContractInfo(
                Translator.getString(
                    R.string.CoinPage_Contract,
                    "ETH"
                ), coinType.address
            )
            is CoinType.Bep20 -> ContractInfo(
                Translator.getString(
                    R.string.CoinPage_Contract,
                    "BSC"
                ), coinType.address
            )
            is CoinType.Bep2 -> ContractInfo(
                Translator.getString(R.string.CoinPage_Bep2Symbol),
                coinType.symbol
            )
            else -> null
        }

    private fun setExtraPageButtons(
        coinMarketItems: List<MarketTickerViewItem>,
        coinInvestorItems: List<InvestorItem>,
        coinVolume: String?
    ) {
        val coinExtraPages = mutableListOf<CoinExtraPage>()
        if (coinVolume != null || coinMarketItems.isNotEmpty()) {
            val listPosition =
                if (coinInvestorItems.isEmpty()) ListPosition.Single else ListPosition.First
            coinExtraPages.add(
                CoinExtraPage.TradingVolume(
                    listPosition,
                    coinVolume,
                    coinMarketItems.isNotEmpty()
                )
            )
        }
        if (coinInvestorItems.isNotEmpty()) {
            val listPosition =
                if (coinMarketItems.isEmpty()) ListPosition.Single else ListPosition.Last
