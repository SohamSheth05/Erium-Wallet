...
import io.horizontalsystems.xrateskit2.entities.CoinCategory
import io.horizontalsystems.xrateskit2.entities.CoinMeta
...
class CoinFragment : BaseFragment(), Chart.Listener, TabLayout.OnTabSelectedListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val drawableResId =
            context?.let { AppLayoutHelper.getCoinDrawableResId(it, viewModel.coinType) }
    ...
    private fun updateNotificationMenuItem() {
        notificationMenuItem?.apply {
            ...
            icon = context?.let {
                val iconRes =
                    if (viewModel.notificationIconActive) R.drawable.ic_notification_24 else R.drawable.ic_notification_disabled
    ...
    private fun observeData() {
        ...
        viewModel.coinDetailsLiveData.observe(viewLifecycleOwner, Observer { item ->
            ...
            if (item.coinMeta.description.isNotBlank()) {
                ...
                aboutTitle.isVisible =
                    item.coinMeta.descriptionType == CoinMeta.DescriptionType.HTML
                val aboutTextSpanned = when (item.coinMeta.descriptionType) {
                    CoinMeta.DescriptionType.HTML -> {
                        Html.fromHtml(
                            item.coinMeta.description.replace("\n", "<br />"),
        ...
        viewModel.setSelectedPoint.observe(viewLifecycleOwner, Observer { item ->
            ...
            pointInfoPrice.text =
                formatter.formatFiat(item.price.value, item.price.currency.symbol, 2, 4)
        ...
        viewModel.showNotificationMenu.observe(
            viewLifecycleOwner,
            Observer { (coinType, coinName) ->
                BottomNotificationMenu.show(
                    childFragmentManager,
                    NotificationMenuMode.All,
                    coinName,
                    coinType
                )
            })
                ...
                val coinInfoView = CoinInfoItemView(context).apply {
                    when (item) {
                            ...
                            if (item.canShowMarkets) {
                                setOnClickListener {
                                    findNavController().navigate(
                                        R.id.coinFragment_to_coinMarketsFragment,
                                        null,
                                        navOptions()
                                    )
                                }
                        ...
                        is CoinExtraPage.Investors -> {
                            ...
                            setOnClickListener {
                                findNavController().navigate(
                                    R.id.coinFragment_to_coinInvestorsFragment,
                                    null,
                                    navOptions()
                                )
                            }
                ...
                if (index == 0) {
                    view.setOnClickListener {
                        MetricChartFragment.show(
                            childFragmentManager,
                            MetricChartType.Coin(viewModel.coinType)
                        )
                    }
                }
    ...
    private fun setLinks(links: Map<LinkType, String>, guideUrl: String?) {
        context?.let { context ->
            ...
            links.onEachIndexed { index, entry ->
                ...
                link.setListPosition(
                    ListPosition.getListPosition(
                        links.size + shiftPosition,
                        index + shiftPosition
                    )
                )
                link.setOnClickListener {
    ...
    private fun setCategoriesAndContractInfo(
