...
sealed class SyncingData {
    data class Blockchain(
        val progress: Int?,
        val spinnerProgress: Int,
        val until: String?,
        val syncingTextVisible: Boolean
    ) : SyncingData()
...
class BalanceViewItemFactory {
    ...
    private fun getRateDiff(item: BalanceModule.BalanceItem): RateDiff {
        val scaledValue = item.latestRate?.rateDiff24h?.setScale(diffScale, RoundingMode.HALF_EVEN)
            ?.stripTrailingZeros()
        val isPositive = (scaledValue ?: BigDecimal.ZERO) >= BigDecimal.ZERO
        val rateDiffText = scaledValue?.let {
