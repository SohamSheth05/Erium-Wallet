...
class ViewHolderTransaction(override val containerView: View, private val l: ClickListener) :
    ...
    fun bind(transactionRecord: TransactionViewItem, showBottomShade: Boolean) {
        ...
        txValueInFiat.setTextColor(
            TransactionViewHelper.getAmountColor(
                transactionRecord.type,
                itemView.context
            )
        )
        val significantDecimal =
            App.numberFormatter.getSignificantDecimalCoin(transactionRecord.coinValue.value)
        txValueInCoin.text = App.numberFormatter.formatCoin(
            transactionRecord.coinValue.value,
            transactionRecord.coinValue.coin.code,
            0,
            significantDecimal
        )
    ...
    fun bindUpdate(current: TransactionViewItem, prev: TransactionViewItem) {
        ...
        if (current.coinValue != prev.coinValue) {
            val significantDecimal =
                App.numberFormatter.getSignificantDecimalCoin(current.coinValue.value)
            txValueInCoin.text = App.numberFormatter.formatCoin(
                current.coinValue.value,
                current.coinValue.coin.code,
                0,
                significantDecimal
            )
        }
    ...
    private fun setBottomIcon(status: TransactionStatus, type: TransactionType) {
        when (status) {
            ...
            is TransactionStatus.Pending -> {
                ...
                bottomIcon.setImageDrawable(image?.let {
