...
class BackupConfirmKeyViewModel(
    ...
    private fun sync(indexItem: BackupConfirmKeyService.IndexItem) {
        val indexViewItem =
            IndexViewItem(first = "${indexItem.first + 1}.", second = "${indexItem.second + 1}.")
            ...
            e.validationErrors.forEach {
                when (it) {
                    BackupConfirmKeyService.ValidationError.EmptyFirstWord -> {
                        firstWordCautionLiveData.postValue(
                            Caution(
                    ...
                    BackupConfirmKeyService.ValidationError.InvalidFirstWord -> {
                        firstWordCautionLiveData.postValue(
                            Caution(
                                Translator.getString(R.string.BackupConfirmKey_Error_InvalidWord),
                                Caution.Type.Error
                            )
                        )
                    }
                    BackupConfirmKeyService.ValidationError.EmptySecondWord -> {
                        secondWordCautionLiveData.postValue(
                            Caution(
                                Translator.getString(R.string.BackupConfirmKey_Error_EmptyWord),
                                Caution.Type.Error
                            )
                    ...
                    BackupConfirmKeyService.ValidationError.EmptyPassphrase -> {
                        passphraseCautionLiveData.postValue(
                            Caution(
                                Translator.getString(R.string.BackupConfirmKey_Error_EmptyPassphrase),
                                Caution.Type.Error
                            )
                        )
                    }
                    BackupConfirmKeyService.ValidationError.InvalidPassphrase -> {
                        passphraseCautionLiveData.postValue(
                            Caution(
                                Translator.getString(R.string.BackupConfirmKey_Error_InvalidPassphrase),
                                Caution.Type.Error
                            )
                        )
                    }
