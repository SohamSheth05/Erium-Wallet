...
class CreateAccountViewModel(
    ...
    val inputsVisibleLiveData = LiveDataReactiveStreams.fromPublisher(
        service.passphraseEnabledObservable.toFlowable(BackpressureStrategy.BUFFER)
    )
    ...
    fun onClickCreate() {
        ...
        try {
            ...
            passphraseConfirmationCautionLiveData.postValue(
                Caution(
    ...
    private fun validatePassphraseAndNotify(
        text: String?,
        cautionLiveData: MutableLiveData<Caution?>
    ): Boolean {
        val valid = service.validatePassphrase(text)
        if (!valid) {
            cautionLiveData.postValue(
                Caution(
