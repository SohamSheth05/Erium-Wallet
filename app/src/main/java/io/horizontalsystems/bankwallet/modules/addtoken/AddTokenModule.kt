    ...
    class Factory(private val tokenType: TokenType) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            val viewModel = when (tokenType) {
                TokenType.Erc20 -> {
                    val resolver = AddErc20TokenResolver(
                        App.buildConfigProvider.testMode,
                        App.appConfigProvider.etherscanApiKey
                    )
                    val blockchainService =
                        AddEvmTokenBlockchainService(resolver, App.networkManager)
                    ...
                    AddTokenViewModel(
                        service,
                        R.string.AddErc20Token_Title,
                        R.string.AddEvmToken_ContractAddressHint
                    )
                }
                TokenType.Bep20 -> {
                    val resolver = AddBep20TokenResolver(
                        App.buildConfigProvider.testMode,
                ...
                TokenType.Bep2 -> {
                    ...
                    AddTokenViewModel(
                        service,
                        R.string.AddBep2Token_Title,
                        R.string.AddBep2Token_TokenSymbolHint
                    )
                }
            }
