...
class AddEvmTokenBlockchainService(
    ...
    override fun coinAsync(reference: String): Single<Coin> {
        val request =
            "api?module=account&action=tokentx&contractaddress=$reference&page=1&offset=1&sort=asc&apikey=${resolver.explorerKey}"

        return networkManager.getEvmInfo(resolver.apiUrl, request)
            .map { response ->
                if (response.get("status").asString == "0") {
                    try {
                        if (response.get("result").asString.toLowerCase(Locale.ENGLISH)
                ...
                return@map Coin(
                    ...
                    type = resolver.coinType(reference)
                )
            }
    }
