...
class SwapCoinCardViewModel(
    ...
    private fun secondaryInfoPlaceHolder(): String? = when (switchService.amountType) {
        AmountType.Coin -> {
            val amountInfo =
                AmountInfo.CurrencyValueInfo(CurrencyValue(fiatService.currency, BigDecimal.ZERO))
    ...
    private fun syncFullAmountInfo(
        ...
        if (fullAmountInfo == null) {
            ...
            val amountString = fullAmountInfo.primaryValue.setScale(decimals, RoundingMode.FLOOR)
    ...
    private fun updateInputFields() {
        ...
        val prefix =
            if (switchService.amountType == AmountType.Currency) fiatService.currency.symbol else null
        val inputParams =
            AmountInputView.InputParams(switchService.amountType, prefix, switchAvailable)
