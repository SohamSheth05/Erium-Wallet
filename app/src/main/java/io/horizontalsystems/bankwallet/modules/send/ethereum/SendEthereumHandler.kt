...
class SendEthereumHandler(
    ...
    private fun syncValidation(): Boolean {
        ...
        val isValid =
            feeModule.isValid && feeModule.feeRateState.isValid && estimateGasLimitState.isValid
    ...
    private fun syncState() {
        ...
        if (feeModule.feeRateState is FeeRateState.Error) {
            ...
            amountModule.setAvailableBalance(
                interactor.availableBalance(
                    (feeModule.feeRateState as FeeRateState.Value).value,
                    (estimateGasLimitState as FeeRateState.Value).value
                )
            )
    ...
    override fun confirmationViewItems(): List<SendModule.SendConfirmationViewItem> {
        return listOf(
            ...
            SendModule.SendConfirmationFeeViewItem(
                feeModule.primaryAmountInfo,
                feeModule.secondaryAmountInfo
            )
        )
    }
