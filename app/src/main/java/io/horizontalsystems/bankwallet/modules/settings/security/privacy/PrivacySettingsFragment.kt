...
class PrivacySettingsFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.showCommunicationSelectorDialog.observe(
            this,
            Observer { (items, selected, coin) ->
                BottomSheetSelectorDialog.show(
                    childFragmentManager,
                    getString(R.string.SettingsPrivacy_CommunicationSettingsTitle),
                    coin.title,
        ...
        viewModel.showCommunicationModeChangeAlert.observe(
            this,
            Observer { (coin, communicationMode) ->
                activity?.let {
                    ConfirmationDialog.show(
                        ...
                        listener = object : ConfirmationDialog.Listener {
                            override fun onActionButtonClick() {
                                viewModel.delegate.proceedWithCommunicationModeChange(
                                    coin,
                                    communicationMode
                                )
                            }
    ...
    private fun getCommunicationModeInfo(communicationMode: CommunicationMode): BottomSheetSelectorViewItem {
        return when (communicationMode) {
            CommunicationMode.Infura -> BottomSheetSelectorViewItem(
                communicationMode.title,
                "infura.io"
            )
