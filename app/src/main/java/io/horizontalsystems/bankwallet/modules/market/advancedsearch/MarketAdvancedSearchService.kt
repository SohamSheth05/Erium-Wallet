...
import io.horizontalsystems.xrateskit2.entities.TimePeriod as XRatesKitTimePeriod
...
class MarketAdvancedSearchService(
    ...
    private fun filterCoinMarket(coinMarket: CoinMarket): Boolean {
        return filterByRange(filterMarketCap, coinMarket.marketInfo.marketCap?.toLong())
                ...
                && (!filterOutperformedBtcOn || outperformed(
            coinMarket.marketInfo.rateDiffPeriod,
            CoinType.Bitcoin
        ))
                && (!filterOutperformedEthOn || outperformed(
            coinMarket.marketInfo.rateDiffPeriod,
            CoinType.Ethereum
        ))
                && (!filterOutperformedBnbOn || outperformed(
            coinMarket.marketInfo.rateDiffPeriod,
            CoinType.Bep2("BNB")
        ))
    }
    ...
    private fun coinMarket(coinType: CoinType): CoinMarket? =
        cache?.firstOrNull { it.data.type == coinType }
