...
class TransactionInfoPresenter(
    ...
    override fun viewDidLoad() {
        ...
        transaction.fee?.let { fee ->
            ...
            viewItems.add(
                TransactionDetailViewItem.Fee(
                    CoinValue(feeCoin, fee),
                    rate?.let { CurrencyValue(it.currency, it.value * fee) })
            )
        }
        ...
        transaction.lockInfo?.originalAddress?.let { recipient ->
            if (transaction.type == TransactionType.Outgoing) {
                viewItems.add(
                    TransactionDetailViewItem.Recipient(
                        transactionInfoAddressMapper.map(
                            recipient
                        )
                    )
                )
    ...
    private fun getExplorerData(
        ...
        return when (coinType) {
            ...
            is CoinType.Erc20 -> TransactionInfoModule.ExplorerData(
                "etherscan.io",
                if (testMode) "https://ropsten.etherscan.io/tx/$hash" else "https://etherscan.io/tx/$hash"
            )
            ...
            CoinType.BinanceSmartChain, is CoinType.Bep20, is CoinType.Dgc, is CoinType.Dsc, is CoinType.Dpc -> TransactionInfoModule.ExplorerData(
                "bscscan.com",
                if (testMode) null else "https://bscscan.com/tx/$hash"
            )
