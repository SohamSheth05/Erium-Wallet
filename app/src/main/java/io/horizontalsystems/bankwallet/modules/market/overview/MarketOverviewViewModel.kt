...
class MarketOverviewViewModel(
    ...
    private fun syncPostsState(postsState: MarketPostService.State) {
        when (postsState) {
            ...
            is MarketPostService.State.Failed -> toastLiveData.postValue(
                convertErrorMessage(
                    postsState.error
                )
            )
    ...
    private fun syncViewItemsBySortingField() {
        topGainersViewItemsLiveData.postValue(
            service.marketItems.sort(SortingField.TopGainers).subList(0, 5)
                .map { MarketViewItem.create(it, MarketField.PriceDiff) })
        topLosersViewItemsLiveData.postValue(
            service.marketItems.sort(SortingField.TopLosers).subList(0, 5)
                .map { MarketViewItem.create(it, MarketField.PriceDiff) })
        if (postService.newsItems.isNotEmpty()) {
            val postViewItems = postService.newsItems.map {
                MarketOverviewModule.PostViewItem(
