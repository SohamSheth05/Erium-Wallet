...
class NotificationsViewModel(
    ...
    private fun loadAlerts() {
        ...
        val deactivateAllButtonEnabled =
            priceAlerts.any { it.trendState != PriceAlert.TrendState.OFF || it.changeState != PriceAlert.ChangeState.OFF }

        setDeactivateButtonEnabled.postValue(deactivateAllButtonEnabled)
        itemsLiveData.postValue(viewItems)
    }

    private fun getPriceAlertViewItems(
        coinName: String,
        coinType: CoinType,
        priceAlert: PriceAlert?
    ): List<NotificationViewItem> {
        ...
        items.add(
            NotificationViewItem(
                coinName,
                NotificationViewItemType.ChangeOption,
                coinType,
                titleRes = R.string.NotificationBottomMenu_Change24h,
                dropdownValue = getChangeValue(priceAlert?.changeState)
            )
        )
        items.add(
            NotificationViewItem(
                coinName,
                NotificationViewItemType.TrendOption,
                coinType,
                titleRes = R.string.NotificationBottomMenu_PriceTrendChange,
                dropdownValue = getTrendValue(priceAlert?.trendState)
            )
        )
        return items
    }
