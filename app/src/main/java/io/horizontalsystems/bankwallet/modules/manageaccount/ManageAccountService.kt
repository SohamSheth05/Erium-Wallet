...
class ManageAccountService(
    ...
    val accountObservable: Flowable<Account> =
        accountSubject.toFlowable(BackpressureStrategy.BUFFER)
    ...
    val accountDeletedObservable: Flowable<Unit> =
        accountDeletedSubject.toFlowable(BackpressureStrategy.BUFFER)

    val accountSettingsInfo: List<Triple<Coin, RestoreSettingType, String>>
        get() {
            val accountWallets = walletManager.getWallets(account)
            return restoreSettingsManager.accountSettingsInfo(account)
                .mapNotNull { (coinType, restoreSettingType, value) ->
