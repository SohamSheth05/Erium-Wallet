...
class BalanceItemViewHolder(
    ...
    private val listener: BalanceItemsAdapter.Listener
    ...
    private fun setRateDiff(rDiff: RateDiff) {
        rateDiff.text =
            rDiff.deemedValue.text ?: containerView.context.getString(R.string.NotAvailable)
    ...
    private fun setTextSyncing(syncingData: SyncingData?) {
        when (syncingData) {
            is SyncingData.Blockchain -> {
                iconProgress.setProgressColored(
                    syncingData.spinnerProgress,
            ...
            is SyncingData.SearchingTxs -> {
                ...
                textSyncedUntil.text = if (syncingData.txCount > 0) {
                    containerView.context.getString(
                        R.string.Balance_FoundTx,
                        syncingData.txCount.toString()
                    )
                } else {
                    null
                }
