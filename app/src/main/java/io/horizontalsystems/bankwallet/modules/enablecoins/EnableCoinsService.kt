...
class EnableCoinsService(
    ...
    private fun resolveTokenType(coinType: CoinType, accountType: AccountType): TokenType? {
        return when {
            ...
            coinType is CoinType.Ethereum && accountType.words.size == 12 -> TokenType.Erc20(
                accountType.words,
                accountType.passphrase
            )
            coinType is CoinType.BinanceSmartChain && accountType.words.size == 24 -> TokenType.Bep20(
                accountType.words,
                accountType.passphrase
            )
            coinType is CoinType.Dgc && accountType.words.size == 24 -> TokenType.Bep20(
                accountType.words,
                accountType.passphrase
            )
            coinType is CoinType.Dsc && accountType.words.size == 24 -> TokenType.Bep20(
                accountType.words,
                accountType.passphrase
            )
            coinType is CoinType.Dpc && accountType.words.size == 24 -> TokenType.Bep20(
                accountType.words,
                accountType.passphrase
            )
            coinType is CoinType.Bep2 && coinType.symbol == "BNB" && accountType.words.size == 24 -> TokenType.Bep2(
                accountType.words,
                accountType.passphrase
            )
            else -> null
    ...
    private fun fetchBep20Tokens(bep20: TokenType.Bep20) {
        val address =
            EthereumKit.address(bep20.words, bep20.passphrase, EthereumKit.NetworkType.BscMainNet)
    ...
    private fun fetchErc20Tokens(erc20: TokenType.Erc20) {
        val networkType =
            if (testMode) EthereumKit.NetworkType.EthRopsten else EthereumKit.NetworkType.EthMainNet
    ...
    private fun handleFetchBep2(tokenSymbols: List<String>) {
        ...
        val coins = tokenSymbols.mapNotNull { symbol ->
            if (symbol.equals("BNB", ignoreCase = true)) {
                allCoins.firstOrNull {
                    it.type is CoinType.Bep2 && (it.type as CoinType.Bep2).symbol.equals(
                        symbol,
                        ignoreCase = true
                    )
                }
