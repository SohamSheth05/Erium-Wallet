...
class BalancePresenter(
    ...
    override fun onItem(viewItem: BalanceViewItem) {
        ...
        if (indexToCollapse != -1) {
            viewItems[indexToCollapse] =
                factory.viewItem(items[indexToCollapse], currency, false, hideBalance)
        }

        if (indexToExpand != -1) {
            viewItems[indexToExpand] =
                factory.viewItem(items[indexToExpand], currency, true, hideBalance)
        }
    ...
    override fun onSyncErrorClick(viewItem: BalanceViewItem) {
        ...
        if (interactor.networkAvailable) {
            view?.showSyncErrorDialog(
                viewItem.wallet,
                errorMessage,
                sourceChangeable(viewItem.wallet.coin.type)
            )
        } else {
        ...
        executor.submit {
            items.forEachIndexed { index, item ->
                latestRate[item.wallet.coin.type]?.let {
                    item.latestRate = it
                    viewItems[index] =
                        factory.viewItem(item, currency, viewItems[index].expanded, hideBalance)
                }
            }
            view?.set(viewItemsCopy)
            updateHeaderViewItem()
    ...
    private fun updateViewItems() {
        ...
        viewItems = items.map {
            factory.viewItem(
                it,
                currency,
                it.wallet == expandedViewItem?.wallet,
                hideBalance
            )
        }.toMutableList()
