...
class ViewHolderMarketOverviewItem(
    ...
    fun bind(item: MarketViewItem, prev: MarketViewItem?, listPosition: ListPosition) {
        ...
        if (item.coinCode != prev?.coinCode) {
            val drawableResId =
                AppLayoutHelper.getCoinDrawableResId(containerView.context, item.coinType)
        ...
        if (item.score != prev?.score) {
            if (item.score == null) {
                ...
                item.score.apply {
                    ...
                    rank.backgroundTintList =
                        ColorStateList.valueOf(getBackgroundTintColor(containerView.context))
                }
                rank.isVisible = true
            }
        }
    ...
    companion object {
        fun create(parent: ViewGroup, listener: Listener): ViewHolderMarketOverviewItem {
            return ViewHolderMarketOverviewItem(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.view_holder_market_overview_item, parent, false), listener
            )
        }
    }
}
