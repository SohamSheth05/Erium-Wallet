...
class AmountInputViewModel(
    ...
    init {
        ...
        fiatService.coinAmountObservable.subscribeIO { syncCoinAmount(it) }
            .let { disposable.add(it) }
        fiatService.inputsUpdatedObservable.subscribeIO { syncInputs() }.let { disposable.add(it) }
        switchService.toggleAvailableObservable.subscribeIO { updateInputFields() }
            .let { disposable.add(it) }
        ...
        when (primaryInfo) {
            is FiatServiceSendEvm.PrimaryInfo.Info -> {
                ...
                if (amountInfo == null || amountInfo.value <= BigDecimal.ZERO) {
                    ...
                    val amount = amountInfo.value.setScale(
                        min(amountInfo.decimal, maxCoinDecimal),
                        RoundingMode.DOWN
                    )
