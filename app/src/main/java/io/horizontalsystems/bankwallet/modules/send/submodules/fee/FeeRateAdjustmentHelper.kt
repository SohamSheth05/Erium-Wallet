...
class FeeRateAdjustmentHelper(private val appConfigProvider: IAppConfigProvider) {
    ...
    fun applyRule(
        coinType: CoinType,
        feeRateAdjustmentInfo: FeeRateAdjustmentInfo,
        feeRate: Long
    ): Long {

        val coinRules =
            rulesByCoin[coinType] ?: return feeRate  //Binance, BCH, Dash, Litecoin has static fee
        ...
        val resolvedCoinAmount: BigDecimal? =
            when (val amountInfo = feeRateAdjustmentInfo.amountInfo) {
