...
class BalanceFragment : BaseFragment(), BalanceItemsAdapter.Listener, ReceiveFragment.Listener,
    ...
    private fun observeLiveData() {
        ...
        viewModel.openSwap.observe(viewLifecycleOwner, Observer { wallet ->
            findNavController().navigate(
                ...
                bundleOf(SwapFragment.fromCoinKey to wallet.coin)
            )
        })
        ...
        viewModel.openSortingTypeDialogLiveEvent.observe(viewLifecycleOwner, Observer { selected ->
            ...
            SelectorDialog
                .newInstance(
                    selectorItems,
                    getString(R.string.Balance_Sort_PopupTitle)
                ) { position ->
        ...
        viewModel.showSyncError.observe(
            viewLifecycleOwner,
            Observer { (wallet, errorMessage, sourceChangeable) ->
