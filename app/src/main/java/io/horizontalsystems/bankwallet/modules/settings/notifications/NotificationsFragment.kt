...
class NotificationItemsAdapter(private val listener: Listener) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            notificationOption -> NotificationItemViewHolder(
                inflate(
                    parent,
                    R.layout.view_holder_setting_with_dropdown,
                    false
                ), onClick = { index -> listener.onItemClick(items[index]) })
            coinName -> NotificationCoinNameViewHolder(
                inflate(
                    parent,
                    R.layout.view_holder_notification_coin_name,
                    false
                )
            )
