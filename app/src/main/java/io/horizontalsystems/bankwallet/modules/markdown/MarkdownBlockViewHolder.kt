...
class ViewHolderFooter(override val containerView: View) : MarkdownBlockViewHolder(containerView),
    LayoutContainer {
    override fun bind(item: MarkdownBlock) {}
}

class ViewHolderH1(override val containerView: View) : MarkdownBlockViewHolder(containerView),
    LayoutContainer {
    override fun bind(item: MarkdownBlock) {
...
class ViewHolderH2(override val containerView: View) : MarkdownBlockViewHolder(containerView),
    LayoutContainer {
    override fun bind(item: MarkdownBlock) {
...
class ViewHolderH3(override val containerView: View) : MarkdownBlockViewHolder(containerView),
    LayoutContainer {
    override fun bind(item: MarkdownBlock) {
...
class ViewHolderImage(override val containerView: View) : MarkdownBlockViewHolder(containerView),
    LayoutContainer {
    ...
    private fun setConstraints(destination: String, mainImage: Boolean) {
        if (containerView is ConstraintLayout) {
            ...
            set.setMargin(
                image.id,
                TOP,
                if (mainImage) 0 else LayoutHelper.dp(12f, containerView.context)
            )
...
class ViewHolderParagraph(
...
) : MarkdownBlockViewHolder(containerView), LayoutContainer {
    ...
    private fun listItem(item: MarkdownBlock) {
        ...
        val bottomPadding =
            if (item.listTightBottom) 0 else LayoutHelper.dp(12f, containerView.context)
