...
class SendFeePresenter(
    ...
    private var feeRateAdjustmentInfo: FeeRateAdjustmentInfo =
        FeeRateAdjustmentInfo(SendAmountInfo.NotEntered, null, baseCurrency, null)
    ...
    private fun validate() {
        ...
        if (availableFeeBalance < fee) {
            throw SendFeeModule.InsufficientFeeBalance(
                baseCoin,
                coinProtocol,
                feeCoin,
    ...
    private fun updateCustomFeeParams(priority: FeeRatePriority.Custom) {
        ...
        val minValue =
            units.coerceAtMost(priority.range.last)   // value can't be more than slider upper range
