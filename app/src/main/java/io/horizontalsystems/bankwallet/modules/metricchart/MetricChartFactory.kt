...
class MetricChartFactory(private val numberFormatter: IAppNumberFormatter) {
    ...
    private fun getFormattedValue(
        ...
        return when (valueType) {
            ...
            MetricChartModule.ValueType.CurrencyValue -> numberFormatter.formatFiat(
                value,
                currency.symbol,
                0,
                2
            )
            MetricChartModule.ValueType.CompactCurrencyValue -> formatFiatShortened(
                value.toBigDecimal(),
                currency.symbol
            )
        }
