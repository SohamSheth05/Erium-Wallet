...
class SwapCoinCardView @JvmOverloads constructor(
    ...
    fun initialize(
        ...
        selectedToken.setOnSingleClickListener {
            val params =
                SelectSwapCoinDialogFragment.params(id, ArrayList(viewModel.tokensForSelection))
        ...
        fragment.getNavigationLiveData(SelectSwapCoinDialogFragment.resultBundleKey)
            ?.observe(lifecycleOwner, { bundle ->
                ...
                val coinBalanceItem =
                    bundle.getParcelable<SwapModule.CoinBalanceItem>(SelectSwapCoinDialogFragment.coinBalanceItemResultKey)
    ...
    private fun observe(viewModel: SwapCoinCardViewModel, lifecycleOwner: LifecycleOwner) {
        ...
        viewModel.amountLiveData().observe(lifecycleOwner, { amount ->
            if (!amountsEqual(
                    ...
                    amountInput.getAmount()?.toBigDecimalOrNull()
                )
            ) {
        ...
        viewModel.secondaryInfoLiveData()
            .observe(lifecycleOwner, { amountInput.setSecondaryText(it) })
