...
class TransactionDetailsAdapter(private val viewModel: TransactionInfoViewModel) :
    RecyclerView.Adapter<TransactionDetailsAdapter.DetailViewHolder>() {
    ...
    class DetailViewHolder(
        ...
        private val viewModel: TransactionInfoViewModel
        ...
        fun bind(detail: TransactionDetailViewItem) {
            ...
            when (detail) {
                ...
                is TransactionDetailViewItem.SentToSelf -> {
                    bindHint(
                        context.getString(R.string.TransactionInfo_SentToSelfNote),
                        iconStart = R.drawable.ic_incoming_20
                    )
                }
        ...
        private fun bindRecipient(detail: TransactionDetailViewItem.Recipient) {
            bindAddress(
                context.getString(R.string.TransactionInfo_RecipientHash),
                detail.recipient
            ) {
        ...
        private fun bindLockInfo(detail: TransactionDetailViewItem.LockInfo) {
            if (detail.lockState.locked) {
                bindHint(
                    context.getString(
                        R.string.TransactionInfo_LockedUntil,
                        DateHelper.getFullDate(detail.lockState.date)
                    ), R.drawable.ic_lock_20, R.drawable.ic_info_20
                )
                ...
                bindHint(
                    context.getString(
                        R.string.TransactionInfo_UnlockedAt,
                        DateHelper.getFullDate(detail.lockState.date)
                    ), iconStart = R.drawable.ic_unlock_20
                )
            }
        }
        ...
        private fun bindDoubleSpend() {
            bindHint(
                context.getString(R.string.TransactionInfo_DoubleSpendNote),
        ...
        private fun bindRate(detail: TransactionDetailViewItem.Rate) {
            val rateFormatted = App.numberFormatter.formatFiat(
                detail.currencyValue.value,
                detail.currencyValue.currency.symbol,
                2,
                4
            )
            val rateValue = itemView.context.getString(
