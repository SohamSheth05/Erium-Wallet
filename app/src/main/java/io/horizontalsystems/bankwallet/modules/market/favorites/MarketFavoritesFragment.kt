...
class MarketFavoritesFragment : BaseFragment(), MarketListHeaderView.Listener,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val emptyListAdapter = EmptyListAdapter(
            marketListViewModel.showEmptyListTextLiveData,
            viewLifecycleOwner
        ) { parent, viewType ->
            EmptyFavoritesViewHolder.create(parent, viewType)
        }

        coinRatesRecyclerView.adapter =
            ConcatAdapter(marketLoadingAdapter, marketItemsAdapter, emptyListAdapter)
    ...
    class EmptyFavoritesViewHolder(containerView: View) : RecyclerView.ViewHolder(containerView) {
        companion object {
            fun create(parent: ViewGroup, viewType: Int): EmptyFavoritesViewHolder {
                return EmptyFavoritesViewHolder(
                    LayoutInflater.from(parent.context)
                        .inflate(R.layout.view_holder_empty_favorites_list, parent, false)
                )
            }
        }
    }
