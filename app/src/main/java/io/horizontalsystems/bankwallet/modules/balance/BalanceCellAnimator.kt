...
object BalanceCellAnimator {
    ...
    fun measureHeights(holder: BalanceItemViewHolder) {
        ...
        expandedHeightBalanceHidden =
            expandedHeight - balanceWrapperHeight - lockedBalanceWrapperHeight
        ...
        collapsedBalanceHiddenHeight =
            expandedHeight - buttonsWrapperHeight - balanceWrapperHeight - borderHeight
    ...
    fun toggleBalanceAndButtons(holder: BalanceItemViewHolder, viewItem: BalanceViewItem) {
        ...
        val bigHeight =
            getHeight(viewItem.expanded, viewItem.hideBalance, viewItem.coinValueLocked.visible)

        holder.balanceCoinLocked.isVisible =
            viewItem.coinValueLocked.visible && !viewItem.hideBalance
        holder.balanceFiatLocked.isVisible =
            viewItem.coinValueLocked.visible && !viewItem.hideBalance

        setExpandProgress(
            holder.balanceWrapper,
            0,
            balanceWrapperHeight,
            if (!viewItem.hideBalance) 1f else 0f
        )
        ...
        setExpandProgress(
            ...
            smallHeight,
            bigHeight,
            if (forwardAnimation) 1f else 0f
        )
    }

    fun toggleBalanceAndButtonsAnimate(
        holder: BalanceItemViewHolder,
        current: BalanceViewItem,
        prev: BalanceViewItem
    ) {
        ...
        rootAnimator.addUpdateListener { valueAnimator ->
            ...
            if (toggleBalance) {
                ...
                if (current.coinValueLocked.visible) {
                    setExpandProgress(
                        holder.lockedBalanceWrapper,
                        0,
                        lockedBalanceWrapperHeight,
                        progress
                    )
                }
            }
        ...
        if (animationForward) { //expand animation
            ...
            rootAnimator.doOnEnd {
                holder.balanceCoinLocked.isVisible =
                    current.coinValueLocked.visible && !current.hideBalance
                holder.balanceFiatLocked.isVisible =
                    current.coinValueLocked.visible && !current.hideBalance
            }
