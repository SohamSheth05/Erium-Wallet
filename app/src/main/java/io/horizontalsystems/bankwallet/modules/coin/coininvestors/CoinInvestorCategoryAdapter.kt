...
class CoinInvestorCategoryAdapter(
    private val items: List<InvestorItem>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            viewTypeFund -> ViewHolderCoinInvestorsItem(
                inflate(
                    parent,
                    R.layout.view_holder_coin_fund_item
                )
            )
            else -> ViewHolderCoinInvestorsSectionHeader(
                inflate(
                    parent,
                    R.layout.view_holder_coin_investors_section_header
                )
            )
        }
    }
...
class ViewHolderCoinInvestorsItem(override val containerView: View) :
