...
class SendEvmTransactionViewModel(
    ...
    private fun getViewItems(
        ...
        when (decoration) {
            ...
            is TransactionDecoration.Eip20Transfer -> getEip20TransferViewItems(
                decoration,
                additionalInfo
            )
    ...
    private fun getTransferViewItems(
        transfer: TransactionDecoration.Transfer,
        additionalInfo: SendEvmData.AdditionalInfo?
    ): List<SectionViewItem> {
        val viewItems = mutableListOf(
            ...
            ViewItem.Value(
                ...
                coinServiceFactory.baseCoinService.amountData(transfer.value).getFormatted(),
                ValueType.Outgoing
            )
        )
    ...
    private fun getEip20TransferViewItems(
        eip20Transfer: TransactionDecoration.Eip20Transfer,
        additionalInfo: SendEvmData.AdditionalInfo?
    ): List<SectionViewItem>? =
        coinServiceFactory.getCoinService(eip20Transfer.contractAddress)?.let { coinService ->
            val viewItems = mutableListOf(
                ViewItem.Subhead(
                    Translator.getString(R.string.Send_Confirmation_YouSend),
                    coinService.coin.title
                ),
                ViewItem.Value(
                    Translator.getString(R.string.Send_Confirmation_Amount),
                    coinService.amountData(eip20Transfer.value).getFormatted(),
                    ValueType.Outgoing
                )
            )
    ...
    private fun getEip20ApproveViewItems(eip20Approve: TransactionDecoration.Eip20Approve): List<SectionViewItem>? =
        coinServiceFactory.getCoinService(eip20Approve.contractAddress)?.let { coinService ->
            ...
            val viewItems = listOf(
                ...
                ViewItem.Value(
                    Translator.getString(R.string.Send_Confirmation_Amount),
                    coinService.amountData(eip20Approve.value).getFormatted(),
                    ValueType.Regular
                ),
                ViewItem.Address(
    ...
    private fun getSwapViewItems(
        swap: TransactionDecoration.Swap,
        additionalInfo: SendEvmData.AdditionalInfo?
    ): List<SectionViewItem>? {
                    ...
                    SectionViewItem(
                        listOf(
                            ViewItem.Subhead(
                                ...
                                coinServiceIn.coin.title
                            ),
                            ViewItem.Value(
                                Translator.getString(R.string.Send_Confirmation_Amount),
                                coinServiceIn.amountData(trade.amountIn).getFormatted(),
                                ValueType.Outgoing
                            )
                        )
                    )
                    ...
                    SectionViewItem(
                        listOf(
                            ViewItem.Subhead(
                                ...
                                coinServiceOut.coin.title
                            ),
                            getEstimatedSwapAmount(info?.let {
                                coinServiceOut.amountData(it.estimatedOut).getFormatted()
                            }, ValueType.Incoming),
                            ViewItem.Value(
                                Translator.getString(R.string.Swap_Confirmation_Guaranteed),
                                coinServiceOut.amountData(trade.amountOutMin).getFormatted(),
                                ValueType.Regular
                            )
                        )
                    )
                ...
                sections.add(
                    SectionViewItem(
                        listOf(
                            ...
                            ViewItem.Value(
                                Translator.getString(R.string.Swap_Confirmation_Maximum),
                                coinServiceIn.amountData(trade.amountInMax).getFormatted(),
                                ValueType.Regular
                            )
                        )
                    )
                ...
                sections.add(
                    SectionViewItem(
                        listOf(
                            ...
                            ViewItem.Value(
                                Translator.getString(R.string.Swap_Confirmation_Guaranteed),
                                coinServiceOut.amountData(trade.amountOut).getFormatted(),
                                ValueType.Regular
                            )
                        )
                    )
    ...
    private fun getFallbackViewItems(transactionData: TransactionData): List<SectionViewItem> {
        ...
        val viewItems = listOf(
            ViewItem.Value(
                Translator.getString(R.string.Send_Confirmation_Amount),
                coinServiceFactory.baseCoinService.amountData(transactionData.value).getFormatted(),
                ValueType.Outgoing
            ),
            ViewItem.Address(
                Translator.getString(R.string.Send_Confirmation_To),
        ...
        when (val convertedError = error.convertedError) {
            ...
            is EvmError.ExecutionReverted -> {
                Translator.getString(
                    ...
                    coinServiceFactory.baseCoinService.coin.code
                )
            }
