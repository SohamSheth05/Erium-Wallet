...
class AddressParserTest {
    ...
    fun parse_BitcoinPaymentAddress() {
        ...
        checkPaymentData(
            addressParser,
            "bitcoin:address_data?amount=0.01?label=test_sender",
            paymentData
        )

        paymentData =
            AddressData(address = "address_data", parameters = mutableMapOf("custom" to "any"))
    ...
    fun parse_BitcoinCashPaymentAddress() {
        ...
        paymentData =
            AddressData(address = "bitcoincash:address_data", version = "1.0", label = "test")
        checkPaymentData(
            addressParser,
            "bitcoincash:address_data;version=1.0?label=test",
            paymentData
        )

        paymentData =
            AddressData(address = "bitcoincash:address_data", amount = "0.01".toBigDecimal())
        checkPaymentData(addressParser, "bitcoincash:address_data?amount=0.01", paymentData)

        paymentData = AddressData(
            address = "bitcoincash:address_data",
            amount = "0.01".toBigDecimal(),
            label = "test_sender"
        )
        checkPaymentData(
            addressParser,
            "bitcoincash:address_data?amount=0.01?label=test_sender",
            paymentData
        )

        paymentData = AddressData(
            address = "bitcoincash:address_data",
            parameters = mutableMapOf("custom" to "any")
        )
    ...
    fun parse_EthereumPaymentAddress() {
        ...
        checkPaymentData(
            addressParser,
            "ethereum:address_data?amount=0.01?label=test_sender",
            paymentData
        )
