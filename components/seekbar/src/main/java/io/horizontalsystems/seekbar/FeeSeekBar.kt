...
class FeeSeekBar @JvmOverloads constructor(
    ...
    private fun initialize() {
        ...
        try {
            ta.getInt(R.styleable.FeeSeekBar_textColor, config.textColor)
                .let { config.textColor = it }
            ta.getInt(R.styleable.FeeSeekBar_textColorSecondary, config.textColorSecondary)
                .let { config.textColorSecondary = it }
            ta.getInt(R.styleable.FeeSeekBar_controlsColor, config.symbolColor)
                .let { config.symbolColor = it }
            ta.getInt(R.styleable.FeeSeekBar_bubbleBackground, config.bubbleBackground)
                .let { config.bubbleBackground = it }
            ta.getInt(R.styleable.FeeSeekBar_bubbleStroke, config.bubbleStroke)
                .let { config.bubbleStroke = it }
        ...
        bubblePrimaryTextPaint.getTextBounds(
            config.bubbleHint,
            0,
            config.bubbleHint.length,
            secTextBounds
        )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        //seekbar line
        canvas.drawLine(
            config.sideSymbolWidth + config.seekbarSideMargin,
            thumbCenter,
            width.toFloat() - config.sideSymbolWidth - config.seekbarSideMargin,
            thumbCenter,
            seekBarLinePaint
        )
