...
class PoolTest {
    ...
    fun getFetchData_hasRecords() {
        ...
        whenever(state.unusedRecords).thenReturn(unusedRecords)
        whenever(state.wallet).thenReturn(wallet)
        whenever(state.records).thenReturn(
            mutableListOf(
                mock(TransactionRecord::class.java),
                lastRecord
            )
        )
        ...
        val fetchData =
            TransactionsModule.FetchData(wallet, lastRecord, limit - unusedRecordsSize + 1)
    ...
    fun add_notEmpty() {
        val records =
            listOf(mock(TransactionRecord::class.java), mock(TransactionRecord::class.java))
