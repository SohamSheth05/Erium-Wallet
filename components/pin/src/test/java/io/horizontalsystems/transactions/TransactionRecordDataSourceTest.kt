...
class TransactionRecordDataSourceTest {
    ...
    fun getFetchDataList() {
        ...
        Assert.assertArrayEquals(
            arrayOf(fetchData1, fetchData2),
            dataSource.getFetchDataList().toTypedArray()
        )
    }
    ...
    fun itemIndexesForTimestamp() {
        ...
        Assert.assertArrayEquals(
            indexes.toIntArray(),
            dataSource.itemIndexesForTimestamp(coin, timestamp).toIntArray()
        )
    }
    ...
    fun allRecords() {
        ...
        val recordsBtc =
            mutableListOf(mock(TransactionRecord::class.java), mock(TransactionRecord::class.java))
        ...
        Assert.assertArrayEquals(
            arrayOf(walletBtc, walletEth),
            actualAllRecords.keys.toTypedArray()
        )
        Assert.assertArrayEquals(
            recordsBtc.toTypedArray(),
            actualAllRecords[walletBtc]?.toTypedArray()
        )
        Assert.assertArrayEquals(
            recordsEth.toTypedArray(),
            actualAllRecords[walletEth]?.toTypedArray()
        )
    }
    ...
    fun handleUpdatedRecords() {
        ...
        whenever(
            factory.createTransactionItem(
                walletBtc,
                insertedRecord1
            )
        ).thenReturn(insertedItem1)

        dataSource.handleUpdatedRecords(records, walletBtc)

        verify(itemsDataSource).handleModifiedItems(
            listOf(updatedItem1, updatedItem2),
            listOf(insertedItem1)
        )
    }
