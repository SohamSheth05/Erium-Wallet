...
class ChartGridTimeline(private val config: ChartConfig, override var isVisible: Boolean = true) :
    ChartDraw {
    ...
    private fun Canvas.drawColumns() {
        columns.forEach {
            drawText(
                it.value,
                textPosition(it.x, it.value),
                config.timelineTextSize + config.timelineTextPadding,
                textPaint
            )
        }
    }
