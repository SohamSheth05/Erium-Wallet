...
class ChartGradient(
    ...
    fun setShader(gradient: ChartConfig.GradientColor) {
        setGradient(
            setAlphaComponent(gradient.startColor, 0x0D),
            setAlphaComponent(gradient.endColor, 0x80)
        )
    }
    ...
    private fun setGradient(colorStart: Int, colorEnd: Int) {
        paint.shader =
            LinearGradient(0f, 0f, shape.width(), 0f, colorStart, colorEnd, Shader.TileMode.REPEAT)
    }
}
