...
class Chart @JvmOverloads constructor(
    ...
    fun setData(
        ...
        val emaFast =
            PointConverter.curve(data.values(EmaFast), chartMain.shape, config.curveVerticalOffset)
        val emaSlow =
            PointConverter.curve(data.values(EmaSlow), chartMain.shape, config.curveVerticalOffset)
        ...
        val signal =
            PointConverter.curve(data.values(MacdSignal), chartBottom.shape, config.macdLineOffset)
        ...
        val coordinates =
            PointConverter.coordinates(data, chartMain.shape, config.curveVerticalOffset)
        val points =
            PointConverter.curve(data.values(Candle), chartMain.shape, config.curveVerticalOffset)
        val volumes =
            PointConverter.volume(data.values(Volume), chartBottom.shape, config.volumeOffset)
        val timeline =
            GridHelper.map(chartType, data.startTimestamp, data.endTimestamp, chartMain.shape.right)
